...
struct HalfEdge {
    ...
    HalfEdge(size_t o, size_t d, size_t f, size_t n, size_t t = static_cast<size_t>(-1))
...
public:
     ...
     */
    HalfEdgeStructure(const std::vector<std::array<size_t, 3>>& faces, size_t num_vertices);
...
private:
     ...
     * @param num_vertices 顶点数量
     */
    void build(const std::vector<std::array<size_t, 3>>& faces, size_t num_vertices);
};
