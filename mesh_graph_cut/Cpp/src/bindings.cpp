...
PYBIND11_MODULE(mesh_graph_cut_cpp, m) {
  ...
  m.def("toSubMeshSamplePoints", &toSubMeshSamplePoints,
        ...
        "    face_groups: list of lists, where each inner list contains face indices for a submesh\n"
        "Returns:\n"
        "    (K, 8192, 3) float array of sampled points for each submesh, where K is the number of face groups");
}