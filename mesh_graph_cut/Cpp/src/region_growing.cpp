    ...
    for (size_t batch = 0; batch < num_batches; ++batch) {
        ...
        futures.push_back(std::async(std::launch::async, [&, start_idx, end_idx]() {
            ...
            for (size_t i = start_idx; i < end_idx; ++i) {
                ...
                // 计算距离
                min_distances[i] = KDTree::distance(vertices[i], seed_points[nearest_seed_idx]);
            }
        }));
    }
...
std::vector<size_t> find_connected_faces(
    ...
    double max_curvature,  // 保留参数以保持接口兼容，但不再使用
    const std::vector<double>& face_curvatures,  // 保留参数以保持接口兼容，但不再使用
...
std::vector<size_t> run_parallel_region_growing(
    ...
    std::cout << "Starting parallel region growing with " << num_segments << " segments" << std::endl;
    ...
    std::vector<std::vector<size_t>> vertex_to_faces = build_vertex_to_face_map(faces, vertices.size());
    ...
    for (size_t i = 0; i < std::min(seed_indices.size(), num_segments); ++i) {
        futures.push_back(std::async(std::launch::async, [&, i]() {
            ...
            // 找到种子点附近的所有面片
            for (size_t face_idx = 0; face_idx < faces.size(); ++face_idx) {
                if (local_visited[face_idx]) continue;
                ...
                if (dist <= radius) {
                    // 查找连通区域，不再使用曲率阈值
                    auto connected = find_connected_faces(
                        face_idx, faces, vertex_to_faces, 
                        std::numeric_limits<double>::max(), // 不使用曲率阈值
                        face_curvatures, local_visited);
    ...
    std::cout << "Parallel region growing completed in " << duration << " ms" << std::endl;
