...
import os
import time
from mesh_cut.Module.smooth_circle_mesh_cutter import SmoothCircleMeshCutter
...
def demo():
    ...
    smooth_circle_mesh_cutter = SmoothCircleMeshCutter(mesh_file_path, dist_max)
    ...
    smooth_circle_mesh_cutter.cutMesh(normal_angle_max, points_per_submesh)
    end_time = time.time()
    print(f"Mesh cutting completed in {end_time - start_time:.2f} seconds")

    print(
        "sub mesh sample points.shape:",
        smooth_circle_mesh_cutter.sub_mesh_sample_points.shape,
    )

    print("Render face labels...")
    smooth_circle_mesh_cutter.renderFaceLabels()
